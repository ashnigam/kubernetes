...
func GetClientCANames(apiHost string) ([]string, error) {
	...
	tlsConfig := &tls.Config{
		InsecureSkipVerify: false, // this is insecure to always get to the GetClientCertificate
...
func GetServingCertificates(apiHost, serverName string) ([]*x509.Certificate, [][]byte, error) {
	tlsConfig := &tls.Config{
		InsecureSkipVerify: false, // this is insecure so that we always get connected
	}
